...
### v1.4.6 — Chores improvements + Nommies fixes
**Chores.** Added a "Start date" field to the add/edit modal for recurring chores, letting you backdate when a chore began rather than always defaulting to today. The field is hidden for one-time tasks since they already have a due date. Background scroll is now locked when the add/edit modal is open (using `body.modal-open { overflow: hidden; position: fixed; }`) and restored on close, swipe-dismiss, or backdrop tap. Input autofill popups suppressed more aggressively by adding unique `name` attributes alongside `autocomplete="off"` to defeat browser contact/address heuristics.

**Nommies (patch).** Map tab was blank due to a broken nested template literal in `initMap` — an emoji ternary inside a string inside a template literal caused a silent JS syntax error that killed the function before rendering. Rewrote using `document.createElement` and string concatenation. Also restored `renderRanking`, `renderStats`, and `tierOf` which were accidentally deleted when the Leaflet block was removed.

**Nommies.** List filter chips (Visited, Want to Go, Favorites, custom lists) were broken: the chip `onclick` inline handler used JS string escaping that some browsers rejected, and click events were silently dropped. Fixed by rewriting `renderListChips` to build chips via `document.createElement` and attach listeners with `addEventListener`, avoiding all inline handler escaping entirely. Added an Address field to the add/edit modal — stored on the restaurant object and used as the primary geocoding query when present (falls back to name + neighborhood). Map tile layer swapped from OpenStreetMap's busy default style to CartoDB Voyager, which is cleaner, higher contrast, and closer to a Google Maps aesthetic — no API key required. Background scroll lock added to all nommies modals (add/edit, import, manage lists) on the same pattern as chores.
